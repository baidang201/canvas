...
import { CanvasStargateClient } from "../../src/checkers_stargateclient"
import { CanvasSigningStargateClient } from "../../src/checkers_signingstargateclient"
...
async function runAll() {
    const client: CanvasStargateClient = await CanvasStargateClient.connect(process.env.RPC_URL)
    ...
    const aliceSigningClient: CanvasSigningStargateClient =
        await CanvasSigningStargateClient.connectWithSigner(process.env.RPC_URL, aliceSigner, {
    ...
    const bobSigningClient: CanvasSigningStargateClient =
        await CanvasSigningStargateClient.connectWithSigner(process.env.RPC_URL, bobSigner, {
            gasPrice: GasPrice.fromString("1stake"),
        })
