mock-expected-keepers:
	mockgen -source=x/canvas/types/expected_keepers.go \
		-package testutil \
		-destination=x/canvas/testutil/expected_keepers_mocks.go 
...
install-protoc-gen-ts:
	...
	curl -L https://github.com/protocolbuffers/protobuf/releases/download/v21.11/protoc-21.11-osx-x86_64.zip -o scripts/protoc/protoc.zip
	cd scripts/protoc && unzip -o protoc.zip
	rm scripts/protoc/protoc.zip

cosmos-version = v0.46.0
...
gen-protoc-ts: 
	...
	ls proto/canvas/canvas | xargs -I {} ./scripts/protoc/bin/protoc \
